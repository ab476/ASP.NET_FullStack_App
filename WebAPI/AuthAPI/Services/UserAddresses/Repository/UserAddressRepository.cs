...
public class UserAddressRepository(AuthDbContext db) : IUserAddressRepository
{
    ...
    public async Task<UserAddressResponse?> UpdateAsync(Guid id, UpdateUserAddressRequest request, CancellationToken ct)
    {
        ...
        var updated = await _db.TUserAddresses
            .TagWith("UserAddressRepository.UpdateAsync.FetchUpdated")
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.Id == id, ct);
